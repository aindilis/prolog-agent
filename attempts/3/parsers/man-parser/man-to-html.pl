...
sub parse_directive_pc
{
   my $self = shift;
   $self->_flush_para;
   $self->chunk( $self->parse_chunks_flat, font => "pc", size => 0 );
}

sub parse_directive_TS
{
   my $self = shift;
   $self->_flush_para;
   $self->chunk( $self->parse_chunks_flat, font => "ts", size => 0 );
}

sub parse_directive_TE
{
   my $self = shift;
   $self->_flush_para;
   $self->chunk( $self->parse_chunks_flat, font => "ts", size => 0 );
