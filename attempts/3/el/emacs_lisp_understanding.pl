...
%% emacsCommand(['prolog-agent-el-get-elisp-function-name-source','\'man'],FunctionSource)

retrieveSourceOfFunctionName(FunctionName,FunctionSource) :-
	functionNameToQuotedFunctionName(FunctionName,QuotedFunctionName),
	view([1]),
	emacsCommandHelper(['prolog-agent-el-get-elisp-function-name-source',QuotedFunctionName],FunctionSource).
...
emacsCommandSample(['prolog-agent-el-get-elisp-function-name-source',QuotedFunctionName],Source) :-
...
emacsLispUnderstanding(FunctionName) :-
	convertFunctionNameToPDDLFileRecursively(FunctionName,FinalPDDLSemantics),
	view([finalPDDLSemantics,FinalPDDLSemantics]).

testPA :-
	emacsLispUnderstanding('kmax-find-function-or-macro-elisp').
